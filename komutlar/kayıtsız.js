...
exports.run = async (client, message, args) => {
  let embed = new Discord.MessageEmbed().setFooter("developed by spatz").setColor('RANDOM');
  ...
  .then(console.log)
  .catch(console.error)
  message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp().setThumbnail(message.author.avatarURL).setFooter(`developed by spatz`).setDescription(`${kisi} üyesi, ${message.author} tarafından kayıtsıza atıldı!`)).catch();// atlas.
};
